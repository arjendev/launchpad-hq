...
|-------|------------|-------------------|-------|
| **Windows / macOS / Linux (bare metal)** | `127.0.0.1` (default) | `ws://localhost:3000` | Everything on same machine |
| **WSL without devcontainers** | `127.0.0.1` (default) | `ws://localhost:3000` | Same Linux instance |
| **HQ in devcontainer, daemon in sibling container** | `0.0.0.0` (auto-detected) | `ws://<hq-container-name>:3000` | Docker bridge network |
| **HQ in devcontainer, daemon on bare WSL/host** | `0.0.0.0` (auto-detected) | `ws://localhost:3000` | VS Code port forwarding |
| **GitHub Codespaces** | `0.0.0.0` (auto-detected) | `ws://localhost:3000` | Same Codespace |
...
**Symptom:** Daemon in one container can't reach HQ in another — connection refused on `localhost:3000`.
...
2. Use the HQ container's hostname instead of `localhost`:
   ```json
   { "hqUrl": "ws://hq-container-name:3000/ws/daemon" }
   ```
